...
site.ScrollFollow = elf().Class({
    ...
    getOriginTop: function() {
        return typeof this.originTop != 'undefined' ? this.originTop : (this.originTop = this.node.getPosition(this.getDocumentElement()).y -
    ...
    getOriginLeft: function() {
        return typeof this.originLeft != 'undefined' ? this.originLeft : (this.originLeft = this.node.getPosition(this.getDocumentElement()).x -
...
elf(function() {
    ...
    elf('a.mail').attr('href', elf().template('mailto:#{0}@#{1}', 'me', 'wilson-zhang.com'));
